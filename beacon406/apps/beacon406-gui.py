...
DBM_OFFSET_DB       = -30.0           # Калибровка в dBm (сдвиг)
PRINT_EVERY_N_SEC   = 1
READ_CHUNK          = 65_536
PULSE_THRESH_DBM    = -50.0
PULSE_STORE_SEC     = 1.5
PSK_YLIMIT_RAD       = 1.5
PSK_BASELINE_MS      = 2.0
EPS = 1e-20
...
# --- State ---
        self.win_samps = max(1, int(round(SAMPLE_RATE_SPS * (RMS_WIN_MS * 1e-3))))
        self.tail_p = np.empty(0, dtype=np.float32)
        self.nco_phase = 0.0
        self.nco_k = 2.0 * np.pi * (BB_SHIFT_HZ / float(SAMPLE_RATE_SPS))
        max_points = int(LEVEL_HISTORY_SEC * SAMPLE_RATE_SPS / max(VIS_DECIM, 1))
        ...
        self.store_max_samps = int(PULSE_STORE_SEC * SAMPLE_RATE_SPS)
...
# --- Multithreading ---
        ...
        self.btn_spec = Button(ax_button_spec, "Спектор")
        ...
        self.btn_params.on_clicked(lambda evt: _on_show_params(self, evt))
            ...
            while True:
                ...
                with self.data_lock:
                    ...
                    if np.any(m) or (seg_end_rel > 0 and seg_start_rel < self.full_samples.size):
                        ...
                        if np.any(m):
                            t0_ms = 1000.0 * (start_abs / float(SAMPLE_RATE_SPS))
                            xs_ms = 1000.0 * (self.full_idx[m] / float(SAMPLE_RATE_SPS)) - t0_ms
                            ys = self.full_rms[m]
                            pulse_data["xs_ms"] = xs_ms
                            pulse_data["ys"] = ys
                            dur_ms = 1000.0 * duration_samps / float(SAMPLE_RATE_SPS)
                        ...
                        if seg_end_rel > 0 and seg_start_rel < self.full_samples.size:
                            ...
                            trim_samps = int(SAMPLE_RATE_SPS * 1e-3)
                            duration_samps = max(1, found_end - start_abs + 1)
                            ...
                            if freq_seg.size >= 8:
                                self.last_impulse_freq_hz = self._measure_frequency_phase(freq_seg, SAMPLE_RATE_SPS)
...
#print("MsgS:",msg_hex)
            ...
            with self.data_lock:
                ...
                xs_top = (idx_end[::step] / float(SAMPLE_RATE_SPS)).astype(np.float64)
        ...
        with self.data_lock:
            if len(self.rms_history) >= 2:
                ...
                self.ax_lvl.set_title(
                    ...
                    f"IF: {IF_OFFSET_HZ:+} Hz | Fs: {SAMPLE_RATE_SPS/1e6:.3f} MS/s | "
