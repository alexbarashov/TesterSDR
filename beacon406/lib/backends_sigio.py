...
# --- SigMF file playback (cf32 / ci16 / ci8 via sigio.open_iq) ---
class SigMFPlaybackBackend(SDRBackend):
    def __init__(self, path: str, **kw):
        super().__init__(**kw)
        self._path = str(path)
        # целевой Fs = что «просит» верхний код (обычно 1e6) — sigio сам сделает целочисленную децимацию
        try:
            from sigio import open_iq  # уже импортирован выше, но на всякий случай
        except Exception as e:
            raise RuntimeError("Требуется lib/sigio.py и пакет sigmf-python (pip install sigmf)") from e

        self._reader = open_iq(self._path, target_fs=self.sample_rate_sps, strict=True)
        st = self._reader.get_status()
        self.actual_sample_rate_sps = float(st["output_sample_rate_sps"])
        self.center_freq_hz = float(st["center_freq_hz"])
        self._eof = False
        self.calib_offset_db = SDR_CALIB_OFFSETS_DB.get("file", 0.0)
    ...
    def read(self, nsamps: int) -> np.ndarray:
        if nsamps <= 0 or self._eof:
            return np.empty(0, dtype=np.complex64)
        x = self._reader.read(int(nsamps))
        if x.size == 0:
            # проверим EOF
            st = self._reader.get_status()
            if st.get("eof"):
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        s.update({
            "driver": "file-sigmf",
            "device_info": f"SigMF: {self._path}",
            "file_path": self._path,
            "sigmf_meta": st.get("sigmf_meta"),
            "eof": bool(getattr(self, "_eof", False)),
        })
        return s
