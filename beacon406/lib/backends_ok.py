...
SDR_DEFAULT_HW_SR = {
    ...
    "sdrplay": 2_000_000.0,
    "airspy": 3_000_000.0,
    "rsa306": 1_000_000.0,  # ~Fs при BW=250 kHz, уточняется через DLL
}
...
class SoapyBackend(SDRBackend):
    ...
    def __init__(self, device_args: Dict[str, Any], **kw):
        ...
        SDR_DEFAULT_HW_SR = {
            ...
            "sdrplay": 2_000_000.0,
            "airspy": 3_000_000.0,
            "rsa306": 437_500.0,  # ~Fs при BW=250 kHz, уточняется через DLL
        }
...
class FilePlaybackBackend(SDRBackend):
    def __init__(self, path: str, **kw):
        ...
        self._data = np.fromfile(path, dtype=np.complex64)
...
def autodetect_soapy_driver() -> str:
    """Возвращает 'soapy_<driver>' по первому найденному устройству (rtlsdr/hackrf/airspy/sdrplay)."""
    try:
        import SoapySDR  # type: ignore
        lst = SoapySDR.Device.enumerate()
        for dev in lst:
            drv = (dev.get('driver') or '').lower()
            if drv in ('rtlsdr', 'hackrf', 'airspy', 'sdrplay'):
                return f"soapy_{drv}"
    except Exception:
        pass
    raise RuntimeError("Soapy: устройства не найдены (rtlsdr/hackrf/airspy/sdrplay). Укажите BACKEND_NAME явно.")
...
class TekRSABackend(SDRBackend):
    def __init__(self,
        ...
        print("======================")
        # NEW: реальный Fs, который даёт RSA (наружу отдаём без ресемпла)
        self.actual_sample_rate_sps = float(fs_val.value)
