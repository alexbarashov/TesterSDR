...
if _telemetry_on:
    ...
    # Verbose режим: дополнительно в консоль
...
class Telemetry:
    """Класс телеметрии для замера задержек в DSP сервисе."""
    ...
    def emit(self, tag, pulse_id=-1, note=None):
        """Запись метки времени с автоматическим расчётом dt_ms."""
        if not self.enabled:
            return
        now_ns = time.perf_counter_ns()
        t_ms = (now_ns - self.t0_ns) / 1e6

        # Расчёт delta от предыдущей метки этого же pulse_id
        ...
        msg += f" sr_sps={self._sr} read_chunk={self._chunk} backend={self._backend} mode={self._mode}"
        if note:
            msg += f' note="{note}"'
        self.log(msg)
...
class BeaconDSPService:
    ...
    def _make_backend(self):
        ...
        # Инициализация контекста телеметрии после создания backend
        ...
        if self.backend:
            ...
            tm._backend = "none"
        # Определяем режим: FILE если backend_name содержит 'file', иначе SDR
    ...
    def _process_samples(self, samples: np.ndarray):
        ...
        if p_cont.size >= self.win_samps:
            ...
            for start_abs, found_end in pairs:
                ...
                # Генерируем pulse_id для телеметрии
                ...
                try:
                    ...
                    # Метка телеметрии: публикация pulse события
