...
# from lib.demod import phase_demod_psk_msg_safe  # disabled: handled by DSP service
# from lib.processing_fm import fm_discriminator  # disabled: handled by DSP service
# from lib.backends import safe_make_backend  # SDR backend support  # disabled: handled by DSP service
...
def _start_dsp_listener():
    if zmq is None:
        print("[WEB] pyzmq not installed — DSP IPC disabled")
        return
...
_req_sock = None
def dsp_send(cmd: dict, timeout=2000):
    global _req_sock
    if zmq is None:
    ...
    if _req_sock is None:
        ...
        _req_sock.RCVTIMEO = timeout
        _req_sock.SNDTIMEO = timeout
    try:
...
def process_samples_realtime(samples: np.ndarray):
    ...
    if p_cont.size >= win_samps:
        ...
        # Калибровка dBm (унифицировано с plot):
        rms_dbm_vec = db10(P_win) + DBM_OFFSET_DB
        try:
            # всегда добавляем калибровку бэкенда, если есть
            rms_dbm_vec += sdr_backend.get_calib_offset_db()
        except Exception:
            # если backend None или без метода — оставляем только DBM_OFFSET_DB
            pass
...
# Глобалы должны существовать выше:
# MIN_PULSE_MS_FOR_PSK = 400.0
# last_reported_end_abs = -1
# in_pulse = False
# pulse_start_abs = 0
# и функция get_actual_fs()

def detect_pulses(rms_dbm_vec: np.ndarray, idx_end: np.ndarray, iq_block: np.ndarray, p_cont_start_idx: int):
    """Однопроходная детекция импульсов (как в plot), без рефратора."""
    global in_pulse, pulse_start_abs, last_reported_end_abs
...
def api_init():
    res = dsp_send({"cmd": "get_status"})
    ok = bool(res.get("ok"))
    st = (res.get("status") or {}) if ok else {}
    return jsonify({
...
def api_state():
    ...
    pulse = DSP_STATE.get("pulse") or {}
    psk = DSP_STATE.get("psk") or {}
    return jsonify({
        "dsp": st,
        "pulse_last": pulse,
        "psk_last": psk,
        "log_len": len(DSP_STATE.get("log") or []),
        "ts": time.time(),
    })
...
def api_last_pulse():
    history = list(DSP_STATE.get("log") or [])
    last_pulse = DSP_STATE.get("pulse")
    last_psk = DSP_STATE.get("psk")
    if last_psk and isinstance(last_psk.get("hex"), str) and len(last_psk["hex"]) > 64:
        last_psk = dict(last_psk)
        last_psk["hex_short"] = last_psk["hex"][:64] + "..."
    return jsonify({
...
def api_set_params():
    ...
    r = dsp_send({"cmd": "set_params", **payload})
