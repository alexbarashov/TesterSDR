...
if _telemetry_on:
    ...
    t_logger = _logging_std.getLogger("telemetry")
    t_logger.setLevel(_logging_std.INFO)

    # RotatingFileHandler
    fh = RotatingFileHandler(
        str(logs_dir / "telemetry.log"),
        maxBytes=2_000_000,
        backupCount=5
    )
    fh.setFormatter(_logging_std.Formatter('%(asctime)s %(message)s', datefmt='%H:%M:%S'))
    t_logger.addHandler(fh)

    # StreamHandler для verbose режима
    if _telemetry_level == "verbose":
        ch = _logging_std.StreamHandler()
        ch.setFormatter(_logging_std.Formatter('%(asctime)s %(message)s', datefmt='%H:%M:%S'))
        t_logger.addHandler(ch)
else:
    import logging as _logging_std
    t_logger = _logging_std.getLogger("telemetry_dummy")

class Telemetry:
    """Класс телеметрии для замера задержек 'импульс → график'."""
    def __init__(self, logger, enabled):
        self.enabled = enabled
        self.log = logger.info if enabled else (lambda *a, **k: None)
        self.t0_ns = time.perf_counter_ns()
        self.last_ns_by_id = {}
        # Контекст (заполняется извне)
        self._sr = 0
        self._chunk = 0
        self._backend = "NA"
        self._mode = "NA"
    ...
    def emit(self, tag, pulse_id=-1, note=None):
        ...
        msg = (f"[TM] tag={tag} t_ms={t_ms:.3f} dt_ms={dt_ms:.3f} pulse_id={pulse_id} "
               f"sr_sps={self._sr} read_chunk={self._chunk} backend={self._backend} mode={self._mode}")
        if note:
            msg += f' note="{note}"'
        self.log(msg)

# Создаём экземпляр телеметрии (контекст заполним позже в __init__)
tm = Telemetry(t_logger, _telemetry_on)
...
# --- Multithreading ---
        ...
        # --- Pulse ID counter for telemetry ---
        self.pulse_id_counter = 0
        self.current_pulse_id = -1  # ID текущего обрабатываемого импульса

        # --- Telemetry: заполняем контекст ---
    ...
    def _process_samples(self, samples: np.ndarray):
        ...
        if p_cont.size >= self.win_samps:
            ...
            if self.in_pulse:
                ...
                # Продолжение импульса - используем сохранённый pulse_id
            else:
                start_abs = None
            ...
            while True:
                ...
                self.in_pulse = False
                self.pulse_start_abs = None
                # Импульс завершён, но current_pulse_id сохраняем для обработки данных
                ...
                with self.data_lock:
                    ...
                    if np.any(m) or (seg_end_rel > 0 and seg_start_rel < self.full_samples.size):
                        pulse_data = {
                            ...
                            "pulse_title": None,
                            "title": None,
                            "pulse_id": self.current_pulse_id  # для телеметрии
                        }
                        if np.any(m):
                ...
                # Обработка импульса завершена, сбрасываем pulse_id
