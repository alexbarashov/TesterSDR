...
if _telemetry_on:
    ...
    t_logger = _logging_std.getLogger("telemetry_dsp2plot")
    t_logger.setLevel(_logging_std.INFO)

    # RotatingFileHandler
    fh = RotatingFileHandler(
        str(logs_dir / "telemetry_dsp2plot.log"),
        maxBytes=2_000_000,
        backupCount=5
    )
...
class Telemetry:
    """Класс телеметрии для замера задержек в dsp2plot клиенте."""
...
class Dsp2PlotUI:
    ...
    def _on_pulse_event(self, obj: Dict[str, Any]):
        ...
        if typ == "pulse":
            ...
            # Сохраняем pulse_id для использования в UI потоке
    ...
    def _update_pulse_plots(self):
        ...
        # Метка телеметрии: начало UI обработки pulse
        ...
        try:
            ...
            if need_draw:
                ...
                # Метка телеметрии: завершение отрисовки
